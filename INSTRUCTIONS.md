...
2. Make sure that you have a JDK installed. Your JDK must be either a Java 8, Java 11, or Java 17 JDK.

3. Check that your setup is correct by running `./gradlew build` (Linux/Mac) 
...
4. Create the lattice structure. The lattice is defined declaratively using files in 
...
5. Implement error reporting. The file `DivByZeroVisitor.java` in the main source
...
6. At this point, your analysis is functional (although not very useful). Verify that 
...
7. Implement the abstraction function. The file `DivByZeroAnnotatedTypeFactory.java` 
...
9. Implement refinement rules. The function `refineLhsOfComparison` in the file
